...
<body class="bg-indigo-50/30 text-slate-900">
            ...
            <div class="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
                ...
                <form id="checkout-form" action="{{ route('checkout.store', $event->id) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="customer_name" id="customer_name" placeholder="Masukkan nama sesuai identitas" class="w-full px-5 py-4 bg-white border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium" required value="{{ old('customer_name') }}">
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2 uppercase tracking-wide">Email Aktif</label>
                            <input type="email" name="customer_email" id="customer_email" placeholder="contoh@gmail.com" class="w-full px-5 py-4 bg-white border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium" required value="{{ old('customer_email') }}">
                        ...
                        <div>
                            ...
                            <input type="tel" name="customer_phone" id="customer_phone" placeholder="08xxxxxxx" class="w-full px-5 py-4 bg-white border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium" required value="{{ old('customer_phone') }}">
                        </div>
                    </div>

                    <button type="button" onclick="validateAndShowMidtrans()" class="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black text-xl shadow-xl shadow-indigo-200 hover:bg-indigo-700 active:scale-95 transition-all">
                        Bayar Sekarang
                    </button>
    ...
    <div id="midtrans-overlay" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 hidden items-center justify-center p-6">
        <div class="bg-white w-full max-w-sm rounded-[2rem] overflow-hidden shadow-2xl animate-bounce-in">
            ...
            <div class="p-8 text-center">
                ...
                <p class="text-xs text-slate-400">Metode Simulasi Pembayaran Otomatis</p>

                <div class="mt-8 space-y-4">
                    <button onclick="submitFormPayment()" class="w-full py-4 border-2 border-indigo-100 rounded-2xl flex justify-between items-center px-6 hover:border-indigo-600 hover:bg-indigo-50/50 transition group">
                        <span class="font-bold group-hover:text-indigo-600">Simulasikan Bayar via QRIS</span>
                        <span class="text-indigo-400">→</span>
                    </button>

                    <button class="w-full py-4 border-2 border-indigo-100 rounded-2xl flex justify-between items-center px-6 opacity-50 cursor-not-allowed text-left">
                        <span class="font-bold text-slate-400">Virtual Account (Disabled)</span>
                    </button>
                </div>
    ...
    <script>
        // Validasi agar user tidak memunculkan midtrans sebelum mengisi form data diri
        function validateAndShowMidtrans() {
        ...
        // Eksekusi pengiriman data form asli ke database saat tombol simulasi QRIS diklik
